...
def title(msg):
    ...
    chromedriver_dir = "C:\\Users\\hongs\\OneDrive\\문서\\chromedriver.exe"
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("노래 재생"))
...
\n-j -> 뮤직봇을 자신이 속한 채널로 부릅니다.
\n-l -> 뮤직봇을 자신이 속한 채널에서 내보냅니다.
\n-URL재생 [노래링크] -> 유튜브URL를 입력하면 뮤직봇이 노래를 틀어줍니다.
(목록재생에서는 사용할 수 없습니다.)
\n-p [노래이름] -> 뮤직봇이 노래를 검색해 틀어줍니다.
\n-s -> 현재 재생중인 노래를 종료하고 다음노래를 틀어줍니다.
...
-listreset -> 목록에 추가된 모든 노래를 지웁니다.
\n-a [노래] -> 노래를 대기열에 추가합니다.
-remove [숫자] -> 대기열에서 입력한 숫자에 해당하는 노래를 지웁니다.""", color = 0x00ff00))
